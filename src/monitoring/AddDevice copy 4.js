...
const AddDevice = () => {
  ...
  const selGunRef = useRef();
  const selGuRef = useRef();
  const [siSel, setSiSel] = useState([]);
  const [gunSel, setGunSel] = useState([]);
  const [guSel, setGuSel] = useState([]);

  const handleSelSi = () => {
    console.log("selref", selSiRef.current.value)
    const selsi = sigungu.filter((items) => items.address.split(" ")[0] === selSiRef.current.value);
    setSiSel(selsi)
    console.log("selsi", siSel)
  }

  const handleSelGun = () => {
    const selgun = sigungu.filter((items) => items.address.split(" ")[1] === selSiRef.current.value);
    setGunSel(selgun)
    console.log("selgun", gunSel)
  }

  const handleSelGu = () => {
    const selgu = sigungu.filter((items) => items.address.split(" ")[2] === selGuRef.current.value);
    setGuSel(selgu)
    console.log("selgu", guSel)
  }
        ...
        <div className='w-2/3 text-lg p-3 flex flex-col'>
          ...
          <div className='bg-slate-50 flex-grow w-full'>
            ...
            <div className='flex pb-6 pt-2 pl-3'>
                ...
                <select onChange={handleSelSi} ref={selSiRef} className='w-32 mr-3'>
                  <option value=''>시</option>
                  {si.map((items) => (
                    <option key={items}>{items}</option>
                  ))}
                </select>
                <select onChange={handleSelGun} ref={selGunRef} className='w-32 mr-3'>
                  <option value=''>군</option>
                  {gun.map((items) => (
                    <option key={items}>{items}</option>
                  ))}
                </select>
                <select onChange={handleSelGu} ref={selGuRef} className='w-32'>
                  <option>구</option>
                </select>
              </div>
