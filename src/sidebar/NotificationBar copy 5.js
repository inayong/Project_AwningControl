...
const Notification = ({ mapData }) => {
  ...
  return (
    <div className="flex h-screen overflow-x-hidden whitespace-nowrap">
      ...
      {isMonitoringPage && (
        <div ref={scrollRef} className={`overflow-auto fixed inset-y-0 right-0 w-96 bg-white shadow-md z-20 transform transition-transform duration-0 ${settingOpen ? 'translate-x-0 opacity-100' : 'translate-x-96 opacity-0'}`}
          style={{ height: isDetailBar ? 'calc(100% - 18rem)' : '100%' }}>
          <div className="p-4">
